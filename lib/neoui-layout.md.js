...
import { addClass, removeClass, hasClass, getStyle, makeDOM } from 'tinper-sparrow/src/dom';
import { on, stopEvent, trigger } from 'tinper-sparrow/src/event';
...
var MDLayout = u.BaseComponent.extend({
    ...
    showMaster: function () {
        if (this._master) {
            if (env.isIE8 || env.isIE9) this._master.style.display = 'none'; //IE下暂时不显示此区域
            else {
    ...
    hideMaster: function () {
        if (this._master) {
            ...
            if (env.isIE8 || env.isIE9) this._master.style.display = 'none';else {
...
function _showPage(el, me) {
    addClass(el, 'current');
    if (!(env.isIE8 || env.isIE9)) el.style.transform = 'translate3d(0,0,0)';
}

function _hidePage(el, me, width) {
    removeClass(el, 'current');
    if (!(env.isIE8 || env.isIE9)) el.style.transform = 'translate3d(' + width + 'px,0,0)';
}

...
async.auto({
    ...
    docPath: function(cb) {
        var prefixdoc = ['global','component','plugin'];
        var i=0;
        for (i=0; i<prefixdoc.length; i++) {
            
            async.forEach(prefixdoc, function(item, callback) { 
                var dir =  path.join(snipPath, item),
                DIR = dir;
                async.auto({
                    kits: function(cb) {
                            ...
                            async.mapSeries(kitfiles, function(kfile, wholeback) {
                                ...
                                if (kfile === snipBase) {
                                    ...
                                    async.mapSeries(demofiles, function(exfile, callback) {
                                        ...
                                        var outexPath = path.join(examplesPath, item,kit + exfile + '.html');

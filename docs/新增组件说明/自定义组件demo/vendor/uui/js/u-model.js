...
App.fn.compsValidate = function (element) {
    ...
    for (var i = 0; i < comps.length; i++) {
        if (comps[i].doValidate){
            var result = comps[i].doValidate({trueValue:true});
...
u.DateTimeAdapter = u.BaseAdapter.extend({
	initialize: function (options) {
		...
		if(this.dataModel){
			...
			if(options.options.startfield){
				this.dataModel.ref(options.options.startfield).subscribe(function(value) {
			...
			if(options.options.startfield){
				var startValue = this.dataModel.getValue(options.options.startfield);
				if(startValue)
